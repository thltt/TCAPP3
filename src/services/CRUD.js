...
// Mọi hàm cần truyền context (c) từ Hono để lấy c.env.mydb

// ===== Thu Chi =====

// Lấy giá trị tồn đầu
export const getAmountStartingBalance = async (c) => {
  const { results } = await c.env.mydb.prepare("SELECT starting_balance FROM settings WHERE id = 1").all();
  return results[0];
};

// Cập nhật giá trị tồn đầu
export const updateStartingBalance = async (c, starting_balance) => {
  const result = await c.env.mydb.prepare("UPDATE settings SET starting_balance = ? WHERE id = 1").bind(starting_balance).run();
  return result;
};

// Thêm giao dịch mới
export const addTransaction = async (c, { date, name, type, amount, category, note }) => {
  const result = await c.env.mydb
    .prepare(
...
export const getTransactionsList = async (c, limit, offset) => {
  const { results } = await c.env.mydb
    .prepare("SELECT * FROM transactions ORDER BY date DESC, id DESC LIMIT ? OFFSET ?")
    .bind(limit, offset)
    .all();
  return results;
};
...
export const countTransactions = async (c) => {
  const { results } = await c.env.mydb.prepare("SELECT COUNT(*) as total FROM transactions").all();
  return results[0].total;
};

// Xóa giao dịch
export const deleteTransaction = async (c, id) => {
  const result = await c.env.mydb.prepare("DELETE FROM transactions WHERE id = ?").bind(id).run();
...
export const getTrips = async (c) => {
  const { results } = await c.env.mydb.prepare("SELECT * FROM phieuchuyen ORDER BY ngay DESC, id DESC").all();
  return results;
};
...
export const removeTrip = async (c, id) => {
  const result = await c.env.mydb.prepare("DELETE FROM phieuchuyen WHERE id = ?").bind(id).run();
...
export const getDebts = async (c) => {
  const { results } = await c.env.mydb.prepare("SELECT * FROM congno ORDER BY ngay DESC, id DESC").all();
  return results;
};
...
export const removeDebt = async (c, id) => {
  const result = await c.env.mydb.prepare("DELETE FROM congno WHERE id = ?").bind(id).run();
  return result.changes;
};
